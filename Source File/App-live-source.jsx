...
function buildDashboardData(dash, invoices, payroll, employees) {
  if (!dash) {
    return { kpi: { income: 0, expenses: 0, profit: 0, outstanding: 0, paidInvoices: 0, overdueCount: 0 },
             chart: [], pl: { income: 0, expenses: 0, salaries: 0, other: 0, profit: 0 },
             invoices: [], salaries: [], attention: [] };
  }

  const k = dash.kpi || {};
  const byDay = {};
  (dash.series?.income || []).forEach(r => {
    byDay[r.d] = { ...(byDay[r.d] || {}), income: Number(r.v) || 0 };
  });
  (dash.series?.invoices || []).forEach(r => {
    byDay[r.d] = { ...(byDay[r.d] || {}), invoices: Number(r.v) || 0 };
  });

  const chart = Object.keys(byDay).sort().map(d => ({
    d: new Date(d).toLocaleDateString("en-IN", { day: "2-digit", month: "short" }),
    income: byDay[d].income || 0,
    invoices: byDay[d].invoices || 0,
    salaries: 0,
  }));

  const recent = (dash.recent || []).map(r => ({
    client: r.client, no: r.no, status: r.status,
    amt: Math.max(0, Number(r.sub || 0) - Number(r.discount || 0)),
  }));

  const salaries = (dash.upcoming || []).map(u => ({
    name: u.name, role: u.role, date: `${u.pay_date}th`, amt: Number(u.amt) || 0,
  }));

  const attention = [];
  if (k.overdueCount > 0) {
    attention.push({ icon: CircleDot, tone: "red",
      title: `${k.overdueCount} invoice${k.overdueCount > 1 ? "s are" : " is"} overdue`,
      sub: `Total outstanding: ${inr(k.outstanding)}` });
  }
  if (salaries.length > 0) {
    attention.push({ icon: Users, tone: "amber",
      title: `${salaries.length} salar${salaries.length > 1 ? "ies" : "y"} pending this month`,
      sub: `Total amount: ${inr(salaries.reduce((a, x) => a + x.amt, 0))}` });
  }

  return {
    kpi: {
      income: Number(k.income) || 0, expenses: Number(k.expenses) || 0,
      profit: Number(k.profit) || 0, outstanding: Number(k.outstanding) || 0,
      paidInvoices: Number(k.paidInvoices) || 0,
      overdueCount: Number(k.overdueCount) || 0,
    },
    chart,
    pl: {
      income: Number(dash.pl?.income) || 0, expenses: Number(dash.pl?.expenses) || 0,
      salaries: Number(dash.pl?.salaries) || 0, other: Number(dash.pl?.other) || 0,
      profit: Number(dash.pl?.profit) || 0,
    },
    invoices: recent, salaries, attention,
  };
}

/* ------------------------------ login screen ----------------------------- */

function LoginScreen({ c, onDone }) {
  ...
  const input = {
    ...
    borderRadius: 10, border: "1px solid #E2E8F0", outline: "none", marginBottom: 12,
  };
    ...
    <div style={{ minHeight: "100vh", background: "#0F172A", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      ...
      <div style={{ width: "100%", maxWidth: 380 }}>
        <div style={{ textAlign: "center", marginBottom: 26 }}>
          <div style={{ fontSize: 24, fontWeight: 800, color: "#fff", letterSpacing: "-0.02em" }}>Digital Hikers</div>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: ".2em", color: A.orange, marginTop: 4 }}>OPS LEDGER</div>
        </div>

        <div style={{ background: "#fff", borderRadius: 18, padding: 26 }}>
          <div style={{ fontSize: 18, fontWeight: 700, color: "#0F172A", marginBottom: 18 }}>Sign in</div>

          <label style={{ fontSize: 12.5, fontWeight: 500, color: "#64748B", display: "block", marginBottom: 6 }}>Username</label>
          <input style={input} value={u} autoFocus autoComplete="username"
            onChange={e => { setU(e.target.value); setErr(""); }}
            onKeyDown={e => e.key === "Enter" && submit()} />

          <label style={{ fontSize: 12.5, fontWeight: 500, color: "#64748B", display: "block", marginBottom: 6 }}>Password</label>
          <input style={input} type="password" value={p} autoComplete="current-password"
          ...
          <button onClick={submit} disabled={busy}
            style={{ width: "100%", padding: 13, borderRadius: 12, background: busy ? "#94A3B8" : "#0F172A",
                     color: "#fff", fontSize: 14, fontWeight: 600, cursor: busy ? "wait" : "pointer" }}>
            {busy ? "Signing in…" : "Sign in"}
          </button>

          <div style={{ marginTop: 18, paddingTop: 14, borderTop: "1px solid #E2E8F0", fontSize: 11.5, color: "#94A3B8", lineHeight: 1.6 }}>
...
export default function App() {
  ...
  /* ---------------- data loading ---------------- */
  const loadAll = async () => {
    const [cl, iv, em, pr] = await Promise.all([
      api.clients(), api.invoices(), api.employees(), api.payroll(),
    ]);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!e.unauthorized) setLoadErr(e.message);
      } finally {
        setBooting(false);
      }
    })();
  ...
  const doLogout = async () => {
    ...
    setUser(null); setClients([]); setInvoices([]); setEmployees([]); setPayroll([]);
    setPage("dashboard");
  };
  ...
  const generateSlip = async (emp) => {
    const r = await guard(() => api.createSlip({
      empId: emp.id, month: salMonth, year: salYear, basic: emp.salary, bonus: 0, deductions: 0,
    }), "Salary slip generated.");
    if (!r) return;
    setPayroll(r.payroll || []);
    setSlipRec((r.payroll || []).find(x => x.id === r.id) || null);
  };

  const createSlip = async (rec) => {
    const r = await guard(() => api.createSlip(rec), "Salary slip created.");
    if (!r) return;
    setPayroll(r.payroll || []);
  ...
  const saveSlip = async (rec) => {
    const r = await guard(() => api.updateSlip(rec), "Slip updated.");
    if (!r) return;
    setPayroll(r.payroll || []);
    setSlipRec((r.payroll || []).find(x => x.id === rec.id) || rec);
  };

  const markSalaryPaid = async (rec, p) => {
    const r = await guard(() => api.markSlipPaid({ id: rec.id, ...p }), "Salary marked as paid.");
    setSalPayFor(null);
    if (!r) return;
    setPayroll(r.payroll || []);
    setSlipRec((r.payroll || []).find(x => x.id === rec.id) || null);
  };
  ...
  const guard = async (fn, okMsg) => {
    ...
    catch (e) { if (e.unauthorized) setUser(null); else say(e.message); return null; }
  ...
  const saveInvoice = async (data) => {
    ...
    let result = null;
    try {
      result = await api.saveInvoice(data);
    } catch (e) {
      if (e.unauthorized) { setBusy(false); setUser(null); return; }
      // Likely an invoice-number collision (e.g. an older invoice this
      // browser didn't know about yet). Fetch the real next number from
      // the server and retry once, silently, instead of surfacing an error.
      try {
        const fresh = await api.nextInvoiceNo();
        if (fresh?.no) result = await api.saveInvoice({ ...data, no: fresh.no });
      } catch { /* fall through */ }
      if (!result) { setBusy(false); say(e.message); return; }
    }
    setBusy(false);
    say("Invoice saved.");
    await loadAll();
    setInvView({ mode: "view", inv: result.invoice || data });
  };

  const updateInvoice = async (id, patch) => {
    if (patch.status) {
      const r = await guard(() => api.setInvoiceStatus(id, patch.status), "Status updated.");
      if (!r) return;
    }
    await loadAll();
  ...
  const addPayment = async (inv, p) => {
    ...
    if (!r) return;
    await loadAll();
    setInvView(v => v.inv && v.inv.id === inv.id ? { ...v, inv: r.invoice } : v);
  };
    ...
    return (
      <div style={{ minHeight: "100vh", background: c.bg, display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 20, fontWeight: 800, color: c.ink }}>Digital Hikers</div>
          <div style={{ fontSize: 12, color: A.orange, letterSpacing: ".18em", fontWeight: 700, marginTop: 3 }}>OPS LEDGER</div>
          <div style={{ fontSize: 13, color: c.muted, marginTop: 14 }}>Loading…</div>
        </div>
      </div>
    );
  }
  ...
  if (!user) {
    return <LoginScreen c={c} onDone={async (u) => {
      setUser(u);
      try { await loadAll(); } catch (e) { setLoadErr(e.message); }
    }} />;
  }
  ...
  return (
    <div className="dh-shell" style={{ background: c.bg, color: c.ink }}>
      ...
      {toast && (
        <div style={{
          position: "fixed", left: "50%", transform: "translateX(-50%)", bottom: 90, zIndex: 90,
          background: c.ink, color: "#fff", fontSize: 13.5, fontWeight: 500,
          padding: "11px 18px", borderRadius: 999, boxShadow: "0 8px 24px rgba(15,23,42,.25)",
        }}>{toast}</div>
      )}
      {busy && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, height: 3, background: A.orange, zIndex: 95, opacity: .9 }} />
      )}
      ...
      <div className="dh-main">
            ...
            <div className="dh-user relative items-center gap-2.5 pl-2.5" style={{ borderLeft: `1px solid ${c.border}` }} ref={userMenuRef}>
              <button onClick={() => setShowUserMenu(v => !v)} className="flex items-center gap-2.5" style={{ background: "transparent" }}>
                ...
                <div className="leading-tight text-left">
                  ...
                  <div className="text-[10px]" style={{ color: c.muted, textTransform: "capitalize" }}>{user.role}</div>
                </div>
              ...
              {showUserMenu && (
                <div style={{
                  ...
                  <button onClick={() => { setShowUserMenu(false); doLogout(); }}
                    style={{ width: "100%", textAlign: "left", padding: "9px 12px", borderRadius: 8, fontSize: 13.5, color: A.clay, fontWeight: 600 }}>
                    Sign out
                  </button>
                </div>
              )}
        ...
        <main className="dh-pad dh-scrollarea" style={{ flex: 1, paddingBottom: 96 }}>
          {page === "dashboard" && (
            <Dashboard c={c} dark={dark} D={D} period={period} setPeriod={setPeriod} customFrom={customFrom} setCustomFrom={setCustomFrom} customTo={customTo} setCustomTo={setCustomTo}
          ...
          {page === "expenses" && (
            <ExpensesPage c={c} dark={dark} expenses={expenses}
              onNew={() => setExpModal({})}
          ...
          {page === "documents" && (
            <DocumentsPage c={c} dark={dark} docs={docs}
              onUpload={() => setShowUpload(true)}
              onDelete={async (d) => {
                if (!window.confirm(`Delete "${d.name}"? This removes the file permanently.`)) return;
                const r = await guard(() => api.deleteDocument(d.id), "Document deleted.");
                if (r) setDocs(prev => prev.filter(x => x.id !== d.id));
              }} />
          )}
          {page === "settings" && (
            <SettingsPage c={c} dark={dark} settings={settings} user={user} banks={banks}
              ...
              onChangePassword={async (cur, next) => {
                try { await api.changePassword(cur, next); return true; }
                catch (e) { return e.message; }
              }} />
          )}
          {page !== "dashboard" && page !== "clients" && page !== "invoices"
            && page !== "employees" && page !== "salary" && page !== "settings"
            && page !== "documents" && page !== "expenses" && (
      ...
      {showEmpForm && (
        <EmployeeForm c={c} onClose={() => setShowEmpForm(false)}
          onSave={async (f) => {
            const r = await guard(() => api.createEmployee(f), "Employee added.");
      ...
      {showSearch && (
        <GlobalSearch c={c} dark={dark}
          data={{ clients, invoices, employees, docs, expenses }}
      ...
      {editClient && (
        <ClientForm c={c} initial={editClient} onClose={() => setEditClient(null)}
          onSave={async (f) => {
            const r = await guard(() => api.updateClient({ ...f, id: editClient.id }), "Client updated.");
            setEditClient(null);
            if (r) await loadAll();
          }} />
      )}
      ...
      {editEmp && (
        <EmployeeForm c={c} initial={editEmp} onClose={() => setEditEmp(null)}
          onSave={async (f) => {
            const r = await guard(() => api.updateEmployee({ ...f, id: editEmp.id }), "Employee updated.");
            setEditEmp(null);
            if (r) await loadAll();
          }} />
      )}
      ...
      {addContactFor && (
        <ContactModal c={c} onClose={() => setAddContactFor(null)}
          onSave={async (f) => {
            const r = await guard(() => api.addContact({ client_id: addContactFor.id, ...f }), "Contact added.");
      ...
      {showUpload && (
        <UploadModal c={c} dark={dark} onClose={() => setShowUpload(false)}
          onSave={async ({ file, name, cat }) => {
            const r = await guard(() => api.uploadDocument(file, name, cat), "Document uploaded.");
            if (!r) return;
            setShowUpload(false);
            try { const dc = await api.documents(); setDocs(dc.documents || []); } catch { /* list refresh */ }
          }} />
      )}
      ...
      {showClientForm && (
        <ClientForm c={c} onClose={() => setShowClientForm(false)}
          onSave={async (f) => {
            const r = await guard(() => api.createClient(f), "Client added.");
      ...
      <div className="dh-bottomnav border-t"
        ...
        <div className="dh-navscroll" ref={navRef}
          onWheel={(e) => {
            if (Math.abs(e.deltaY) > Math.abs(e.deltaX)) navRef.current.scrollLeft += e.deltaY;
          }}>
          {nav.map(n => {
            const on = page === n.id;
            return (
...
function Dashboard({ c, dark, D, period, setPeriod, customFrom, setCustomFrom, customTo, setCustomTo, onNewInvoice, onViewInvoices, onNewSlip, onAddExpense, onUploadDoc }) {
  const kpis = [
    { label: "Total Income",  value: inr(D.kpi.income),      Icon: PiggyBank, tone: "green",
      note: "Payments received", noteColor: c.muted },
    { label: "Total Expenses",value: inr(D.kpi.expenses),    Icon: Package,   tone: "red",
      note: "Salaries paid", noteColor: c.muted },
    { label: "Net Profit",    value: inr(D.kpi.profit),      Icon: BarChart3, tone: "blue",
      note: D.kpi.profit >= 0 ? "Income minus expenses" : "Running at a loss",
      noteColor: D.kpi.profit >= 0 ? c.muted : A.red },
    { label: "Outstanding",   value: inr(D.kpi.outstanding), Icon: Clock,     tone: "amber",
      note: D.kpi.overdueCount > 0 ? `${D.kpi.overdueCount} invoice${D.kpi.overdueCount > 1 ? "s" : ""} overdue` : "Nothing overdue",
      noteColor: D.kpi.overdueCount > 0 ? A.red : c.muted },
    ...
    <div className="mx-auto" style={{ maxWidth: 1560 }}>
      ...
      <div className="dh-mid">
        <Card c={c} className="p-5">
          ...
          {period === "Custom Range" && (
            <div className="flex flex-wrap items-center gap-2 mb-4">
              <span className="text-[12.5px] font-medium" style={{ color: c.inkSoft }}>From</span>
              <div style={{ width: 160 }}><DateField c={c} value={customFrom} onChange={setCustomFrom} placeholder="Start date" /></div>
              <span className="text-[12.5px] font-medium" style={{ color: c.inkSoft }}>To</span>
              <div style={{ width: 160 }}><DateField c={c} value={customTo} onChange={setCustomTo} placeholder="End date" /></div>
            </div>
          )}
        ...
        <Card c={c} className="p-5">
          ...
          <div className="mt-4">
            {[
              ["Total Income",  inr(D.pl.income),      c.ink],
              ["Total Expenses","– " + inr(D.pl.expenses), c.ink],
              ["Salaries Paid", "– " + inr(D.pl.salaries), c.ink],
              ["Other Expenses","– " + inr(D.pl.other),    c.ink],
            ].map(([l, v, col], i) => (
          ...
          <div className="flex items-center justify-between rounded-xl px-4 py-3.5 mt-2"
            ...
            <span className="text-[19px] font-bold" style={{ color: A.green }}>{inr(D.pl.profit)}</span>
          </div>
        </Card>
          ...
          {D.attention.length === 0 ? (
            ...
            <button key={a.title} className="w-full flex items-center gap-3.5 p-3.5 mb-3 rounded-xl text-left"
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-[12.5px] mt-0.5" style={{ color: c.muted }}>{a.sub}</div>
              </div>
...
function ClientProfile({ c, dark, client: raw, invoices: allInvoices, co, banks, onBack, onEdit, onAddNote, onAddContact, onDelete, onUploadDoc, onDeleteDoc }) {
  ...
  const client = {
    ...
    since: raw.since ? fmtDate(raw.since) : "—",
    documents: raw.documents || [],
    contacts: raw.contacts || [],
    noteList: (raw.note_list || raw.noteList || []).map(n => ({
      text: n.body ?? n.text, by: n.author ?? n.by,
      date: n.created_at ? fmtDate(String(n.created_at).slice(0, 10)) : (n.date || ""),
    })),
    activity: (raw.activity || []).map(a => ({
      text: a.body ?? a.text, tone: a.tone || "indigo",
...
function InvoiceEditor({ c, dark, co, invoice, clients, invoices, banks, onCancel, onSave }) {
  ...
  // Client-side guess only — used as an instant placeholder. It can be wrong if
  // there's an older invoice the browser doesn't know about yet (e.g. one made
  // before this app was connected, or from another device). The real, authoritative
  // number is fetched from the server right after mount and swapped in below.
  const nextNo = () => {
    const year = new Date().getFullYear();
    const n = invoices.filter(i => (i.no || "").includes(`-${year}-`)).length + 1;
    return `${co.prefix}-${year}-${String(n).padStart(3, "0")}`;
  };
  ...
  useEffect(() => {
    if (!isNew) return;
    api.nextInvoiceNo().then(r => {
      if (r?.no) setF(prev => ({ ...prev, no: r.no }));
    }).catch(() => { /* keep the client-side guess if the server call fails */ });
    ...
    <div style={{ maxWidth: 1560, margin: "0 auto" }}>
      ...
      <div className="dh-profile">
        <Card c={c} style={{ padding: 24, alignSelf: "start" }}>
          ...
          {[["Department", emp.dept || "—"], ["Joined", fmtDate(emp.joinDate || emp.join_date)],
          ...
          {tab === "Salary History" && (
            <Card c={c} style={{ padding: 20 }}>
              ...
              {(emp.history || []).length === 0 && (
                <Blank c={c} Icon={Wallet} title="No salary history yet" sub="It builds up as you generate slips." />
              )}
              {(emp.history || []).map((h, n) => (
                <div key={n} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 0", borderBottom: `1px solid ${c.border}` }}>
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 600 }}>{inr(h.basic ?? h.amt)} / month</div>
                    <div style={{ fontSize: 12.5, color: c.muted, marginTop: 2 }}>
                      {h.month ? `${MONTHS[h.month - 1]} ${h.year}` : `Effective from ${h.from}`}
                    </div>
                  </div>
        ...
        <div style={{ padding: 22 }}>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
            ...
            <button onClick={() => f.name && onSave({ ...f, salary: Number(f.salary) || 0 })}
              style={{ flex: 1, padding: 12, borderRadius: 12, background: A.orange, color: "#fff", fontSize: 14, fontWeight: 600 }}>Save employee</button>
          </div>
        </div>
      </div>
    </div>
...
function SlipDoc({ c, co = COMPANY, rec, emp }) {
    ...
    <div className="print-area" style={{ background: "#fff", color: "#0F172A", borderRadius: 16, border: `1px solid ${c.border}`, padding: 28 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}>
        <div>
          ...
          {co.address && <div style={{ fontSize: 11.5, color: "#64748B", marginTop: 3 }}>{co.address}</div>}
        </div>
...
function SlipView({ c, dark, co, rec, emp, onBack, onSave, onMarkPaid }) {
  ...
  return (
    <div style={{ maxWidth: 1560, margin: "0 auto" }}>
      ...
      <div className="dh-editor">
        <SlipDoc c={c} co={co} rec={f} emp={emp} />
              ...
              <tbody>
                {shown.map(d => (
                  <tr key={d.id} style={{ borderBottom: `1px solid ${c.border}` }}>
                    ...
                    <Td c={c} align="right">
                      <span style={{ display: "inline-flex", gap: 14, color: c.muted }}>
                        <a href={api.documentUrl(d.id)} target="_blank" rel="noopener noreferrer" style={{ color: c.muted, display: "flex" }}>
                          <Download size={17} />
                        </a>
            ...
            <div className="dh-cards">
              {shown.map(d => (
                <div key={d.id} style={{ padding: 16, borderBottom: `1px solid ${c.border}`, display: "flex", gap: 12, alignItems: "center" }}>
                  ...
                  <a href={api.documentUrl(d.id)} target="_blank" rel="noopener noreferrer" style={{ color: c.muted }}>
                    <Download size={17} />
                  </a>
                </div>
              ))}
        ...
        <div style={{ padding: 22 }}>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 18 }}>
            ...
            <button disabled={!file || busy}
              onClick={async () => { setBusy(true); await onSave({ file, name: name || file.name, cat }); setBusy(false); }}
              style={{
                flex: 1, padding: 12, borderRadius: 12, fontSize: 14, fontWeight: 600, color: "#fff",
